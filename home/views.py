...
def delete_comp(request, id):
    if request.method == 'POST':
        pi = Company.objects.get(id=id)
        pi.delete()
        return redirect('/company') 

# function for delete Sales 
def delete_sales(request, id):
    if request.method == 'POST':
        pi = Sales.objects.get(id=id)
        pi.delete()
...
# function for delete Suppliers
def delete_supp(request, id):
    if request.method == 'POST':
        pi = Supplier.objects.get(id=id)
        pi.delete()
...
# function for delete SupplierInvoice 
def delete_supp_inv(request, id):
    if request.method == 'POST':
        pi = SupplierInvoice.objects.get(id=id)
        pi.delete()
...
def dashboard(request):
    ...
    total_conts = Contact.objects.count()
    total_emps = Employee.objects.count()
    total_supps = Supplier.objects.count()
    total_suppins = SupplierInvoice.objects.count()


    sales = Sales.objects.all()
    total_sales = sum(sales.values_list('amount', flat=True))
